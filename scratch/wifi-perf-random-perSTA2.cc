// =============================================================
//   Wifi-pref-random.cc - MODIFIED FOR REDUCED PACKET LOSS
//   - Optimized traffic parameters
//   - Better PHY configuration
//   - Improved channel settings
// =============================================================
                     ...
                     double interval)
{
    ...
    int delta = std::round(rnd->GetValue(-2.0, 2.0));  // REDUCED: smaller density changes
...
// ========================================================
// Wi-Fi BSS setup - IMPROVED FOR LOWER PACKET LOSS
// ========================================================
struct WifiBss
{
             ...
             bool indoor)
{
    ...
    // IMPROVED: Increased TX power and better receiver sensitivity
    phy.Set("TxPowerStart", DoubleValue(txPower));
    phy.Set("TxPowerEnd",   DoubleValue(txPower));
    phy.Set("RxSensitivity", DoubleValue(-92.0));  // Better sensitivity (default is -93.59)
    phy.Set("CcaEdThreshold", DoubleValue(-82.0)); // Better CCA threshold
    phy.Set("TxGain", DoubleValue(3.0));           // Add antenna gain
    phy.Set("RxGain", DoubleValue(3.0));

    WifiMacHelper mac;
    Ssid ssid = Ssid(subnet);

    // IMPROVED: Better MAC parameters for STAs
    mac.SetType("ns3::StaWifiMac",
                "Ssid", SsidValue(ssid),
                "ActiveProbing", BooleanValue(false),
                "MaxMissedBeacons", UintegerValue(10));  // More tolerance
    b.staDevs = wifi.Install(phy, mac, b.stas);

    // IMPROVED: Better MAC parameters for AP
    mac.SetType("ns3::ApWifiMac",
                "Ssid", SsidValue(ssid),
                "BeaconInterval", TimeValue(MicroSeconds(102400)),  // 100ms = 102400us (multiple of 1024)
    ...
    ms.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
                        ...
                        RectangleValue(Rectangle(-areaHalf, areaHalf,
                                                 -areaHalf, areaHalf)),
                        "Speed", StringValue("ns3::ConstantRandomVariable[Constant=1.5]"),  // Slower movement
                        "Mode", EnumValue(RandomWalk2dMobilityModel::MODE_TIME),
                        "Time", TimeValue(Seconds(2.0)));  // Change direction less frequently
    ms.Install(b.stas);
...
// ========================================================
// UDP traffic - IMPROVED TO REDUCE CONGESTION
// ========================================================
...
{
    uint32_t nMin      = 5;
    uint32_t nMax      = 20;       // REDUCED: Lower max nodes
    double   simTime   = 30.0;
